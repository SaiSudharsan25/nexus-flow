...
export default function Dashboard() {
  ...
  const handleAddTask = async (status: 'todo' | 'in-progress' | 'done' = 'todo') => {
    try {
      const tasksRef = collection(db, 'tasks');
      await addDoc(tasksRef, {
        title: "New Project Task",
        description: "Specify task details and sync with Google Drive.",
        tag: "Marketing",
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden animate-fade-in">
      {/* Premium Header */}
      <header className="h-24 bg-white/80 backdrop-blur-xl border-b border-gray-200/50 flex items-center justify-between px-10 shadow-[0_1px_3px_rgba(0,0,0,0.02)] z-10">
        <div className="flex flex-col">
          <div className="flex items-center space-x-3 mb-1">
            <h2 className="text-2xl font-black text-gray-900 tracking-tight">Main Project Canvas</h2>
          ...
          <p className="text-xs font-semibold text-gray-400">Collaboration Board for Team Prompt Wars</p>
        </div>
        ...
        <div className="flex items-center space-x-6">
          ...
          <button onClick={() => handleAddTask()} className="h-11 bg-primary hover:bg-primary-hover text-white px-6 rounded-2xl font-bold text-sm transition-all shadow-xl shadow-primary/20 flex items-center space-x-2 active:scale-95">
      ...
      {/* Main Board Area */}
      <div className="flex-1 overflow-x-auto p-10 bg-[#FDFDFD]">
        <div className="flex space-x-8 h-full min-w-max items-start">
          
          {columns.map(col => (
            <div key={col.id} className="w-[360px] flex-shrink-0 flex flex-col h-full group/column">
              {/* Column Header */}
              <div className="flex items-center justify-between mb-6 px-2">
                <div className="flex items-center space-x-3">
                  <div className={`w-3 h-3 rounded-full ${col.color} shadow-[0_0_12px_rgba(0,0,0,0.1)]`}></div>
                  <h3 className="font-black text-sm text-gray-800 uppercase tracking-widest">{col.title}</h3>
                </div>
                <div className="flex items-center space-x-2">
                  ...
                  <button onClick={() => handleAddTask(col.id as any)} className="w-8 h-8 rounded-xl hover:bg-gray-100 flex items-center justify-center text-gray-400 hover:text-gray-600 transition-all opacity-0 group-hover/column:opacity-100">
              ...
              <div className="flex-1 overflow-y-auto space-y-5 pr-2 custom-scrollbar pb-10">
                {loading && (
                  <div className="flex flex-col items-center justify-center py-20 opacity-40">
                    ...
                    <p className="text-xs font-bold text-gray-500 uppercase tracking-widest">Initializing...</p>
                  </div>
                )}
                ...
                {!loading && filteredTasks.filter(t => t.status === col.id).length === 0 && (
                  <div className="text-center py-12 bg-gray-50/50 rounded-3xl border-2 border-dashed border-gray-200/50">
                    <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">No tasks found</p>
                  </div>
                )}
                ...
                >
                  ...
                  <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest group-hover:text-primary transition-colors">Create New Task</span>
                </button>
              </div>
            </div>
          ))}
